 ...
 *
 * This structure wraps an [OutputSink] and writes zlib-compressed data to it
...
public class ZlibWriteEncoder<W : OutputSink> private constructor(
    ...
    /** Flushes the compressor, writing any buffered output to the underlying sink. */
    public fun flush() {
        inner.flush()
    }
}
 ...
 * as zlib-compressed data is written to this decoder.
 *
 * After decoding a single zlib member, subsequent writes return zero
...
public class ZlibWriteDecoder<W : OutputSink> private constructor(
    ...
    public fun reset(newSink: W): W {
        inner.finish()
        inner.data = DecompressOps(Decompress.new(zlibHeader = true))
        return inner.replace(newSink)
    }
    ...
    /** Flushes the decompressor, writing any buffered output. */
    public fun flush() {
        inner.flush()
    }
}