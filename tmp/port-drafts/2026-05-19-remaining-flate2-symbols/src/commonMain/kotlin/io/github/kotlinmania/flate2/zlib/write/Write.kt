...
package io.github.kotlinmania.flate2.zlib.write
...
public object WImpl {
    ...
    // be flushed.
    public fun new(w: W, level: io.github.kotlinmania.flate2.Compression): ZlibEncoder<W> {
        ZlibEncoder {
    ...
    public fun newWithCompress(w: W, compression: Compress): ZlibEncoder<W> {
        ZlibEncoder {
...
// to the end of the ZLIB member and subsequent writes will return Result.success(0) allowing the caller to
...
public object WImpl4 {
    ...
    // be flushed.
    public fun new(w: W): ZlibDecoder<W> {
        ZlibDecoder {
    ...
    // When this decoder is dropped or unwrapped the final pieces of data will
    // be flushed.
    public fun newWithDecompress(w: W, decompression: Decompress): ZlibDecoder<W> {
        ZlibDecoder {
    ...
    public fun reset(w: W): Result<W> {
        this.inner.finishUnit
        this.inner.data = Decompress.new(true)
        Result.success(this.inner.replace(w))
    }
...
object Tests {
    ...
    private fun decodeExtraDataUnit: Unit {
        val compressed = {
            var e = ZlibEncoder.new(mutableListOf<Any>Unit, Compression.defaultUnit)
        ...
        var decoder = ZlibDecoder.new(writer)
        var consumedBytes = 0
        loop {
