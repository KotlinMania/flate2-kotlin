...
val repoPackage = "io.github.kotlinmania.flate2"
...
kotlin {
    ...
    val xcf = XCFramework("Flate2")

    // Apple desktop
    macosArm64 {
        binaries.framework { baseName = "Flate2"; xcf.add(this) }
    }
    ...
    swiftExport {
        moduleName = "Flate2"
        flattenPackage = repoPackage
    }

    android {
        namespace = repoPackage
        compileSdk = 34
        minSdk = 24
...
mavenPublishing {
    ...
    coordinates(group.toString(), "flate2-kotlin", version.toString())

    pom {
        name.set("flate2-kotlin")
        description.set("Kotlin Multiplatform port of rust-lang/flate2-rs - DEFLATE compression and decompression exposed as Read/BufRead/Write streams. Supports miniz_oxide and multiple zlib implementations. Supports zlib, gzip, and raw deflate streams")
        inceptionYear.set("2026")
        url.set("https://github.com/KotlinMania/flate2-kotlin")
        ...
        scm {
            url.set("https://github.com/KotlinMania/flate2-kotlin")
            connection.set("scm:git:git://github.com/KotlinMania/flate2-kotlin.git")
            developerConnection.set("scm:git:ssh://github.com/KotlinMania/flate2-kotlin.git")
        }
    }
}
...
val codeqlCompileJvm = tasks.register<JavaExec>("codeqlCompileJvm") {
    ...
    doFirst {
        ...
        val sourceFiles = if (sources.files.isEmpty()) {
            ...
            emptySourceSentinel.get().asFile.delete()
